...
const { getAllPenyakit, getContentFirst, getContent } = require('./scraper')
        ...
        switch(intention.name) {
            ...
            case 'tanya_definisi': {
                try
                {
                    ...
                    let content = await getContentFirst(`${process.env.ROOT_URL}/${nama_penyakit.body.toLowerCase().replace(' ', '-')}`)
                    if (!content)
                    {
                        ...
                        content = await getContentFirst(`${found.source_url}`)
                    }
                    return content
                }
                catch (err)
